...
namespace FaceDominatorCore.FDViewModel.MessageViewModel
{
    ...
    public class BrodcastMessageViewModel : BindableBase
    {
        public BrodcastMessageViewModel()
        {
            ...
            BrodcastMessageModel.GenderAndLocationFilter.IsFriendsDropdownVisible = true;
            ...
            {
                BrodcastMessageModel.ListQueryType.Add(item: Application.Current.FindResource(query.GetDescriptionAttr())?.ToString());
            });
        ...
        private void SelectionChangedCommandExecute(object sender)
        {
            ...
            BrodcastMessageModel.GenderAndLocationFilter.IsGroupCategoryEnabled =
                messageData.CurrentQuery.QueryType == Application.Current.FindResource("LangKeyGroupMembers")?.ToString()
                || messageData.ListQueryInfo.Any(x => x.QueryType == Application.Current.FindResource("LangKeyGroupMembers")?.ToString());

            BrodcastMessageModel.GenderAndLocationFilter.IsTimeLimitEnabled =
                messageData.CurrentQuery.QueryType == Application.Current.FindResource("LangKeyPeopleConnectedInMessenger")?.ToString()
                || BrodcastMessageModel.SavedQueries.Any(x => x.QueryType == Application.Current.FindResource("LangKeyPeopleConnectedInMessenger")?.ToString());
        ...
        private void CustomFilter(object sender)
        {
            try
            {
                var moduleSettingsUserControl = sender as ModuleSettingsUserControl<BrodcastMessageViewModel, BrodcastMessageModel>;
        ...
        private void AddQuery(object sender)
        {
            try
            {
                ...
                if (moduleSettingsUserControl != null)
                {
                    ...
                    listQuery.ForEach(z =>
                    {
                        moduleSettingsUserControl?.SetQueryTypeEnumName(Enum.GetNames(typeof(FdProfileQueryParameters)), moduleSettingsUserControl._queryControl.CurrentQuery);
                                                          ...
                                                          moduleSettingsUserControl._queryControl.QueryCollection.Count != 0))
                {
                    moduleSettingsUserControl._queryControl.QueryCollection.ForEach(query =>
                    {
                        ...
                        moduleSettingsUserControl?.SetQueryTypeEnumName(Enum.GetNames(typeof(FdProfileQueryParameters)), currentQuery);
                                ...
                                x.Content.QueryType == currentQuery.QueryTypeDisplayName))
                        {
                            Model.ManageMessagesModel.LstQueries.Add(new QueryContent
                            {
                                ...
                                {
                                    QueryValue = query,
                                    QueryTypeDisplayName = currentQuery.QueryTypeDisplayName,
                ...
                BrodcastMessageModel.GenderAndLocationFilter.IsGroupCategoryEnabled =
                    BrodcastMessageModel.SavedQueries.Any(x => x.QueryType == Application.Current.FindResource("LangKeyGroupMembers")?.ToString());
        ...
        private void AddMessages(object sender)
        {
            ...
            if (string.IsNullOrEmpty(messageData.Messages.MessagesText?.Trim()) && messageData.Messages.Medias?.Count == 0)
            {
                Dialog.ShowDialog(Application.Current.MainWindow, "LangKeyWarning".FromResourceDictionary(),
