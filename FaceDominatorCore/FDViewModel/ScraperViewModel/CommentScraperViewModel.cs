...
namespace FaceDominatorCore.FDViewModel.ScraperViewModel
{
    public class CommentScraperViewModel : BindableBase
    {
        public CommentScraperViewModel()
        {
            ...
            Enum.GetValues(typeof(CommentScraperParameter)).Cast<CommentScraperParameter>().ToList().ForEach(query =>
            {
        ...
        private void AddQuery(object sender)
        {
            try
            {
                ...
                moduleSettingsUserControl?.AddQuery(typeof(CommentScraperParameter));
            }
            catch (Exception ex)
            {
                ex.DebugLog();
