...
{
    public class WatchPartyInviterViewModel : BindableBase
    {

        public WatchPartyInviterViewModel()
        {
            if (WatchPartyInviterModel.SelectAccountDetailsModel == null)
            {
                WatchPartyInviterModel.SelectAccountDetailsModel = new SelectAccountDetailsModel();
            }

            WatchPartyInviterModel.JobConfiguration = new JobConfiguration
            {
        ...
        private void SaveCommandExecute(object sender)
        {
            try
            {
                if (!string.IsNullOrEmpty(WatchPartyInviterModel.InviterDetailsModel.WatchPartyUrls))
                    WatchPartyInviterModel.InviterDetailsModel.ListWatchPartyUrls =
                        Regex.Split(WatchPartyInviterModel.InviterDetailsModel.WatchPartyUrls, "\r\n").ToList();
            }
            catch (Exception ex)
            {
                ex.DebugLog();
        ...
        public WatchPartyInviterModel Model => WatchPartyInviterModel;

        private WatchPartyInviterModel _watchPartyInviterModel = new WatchPartyInviterModel();

        public WatchPartyInviterModel WatchPartyInviterModel
        {
            get
            {
                return _watchPartyInviterModel;
            }
            set
            {
                if (_watchPartyInviterModel == null & _watchPartyInviterModel == value)
                    return;
                SetProperty(ref _watchPartyInviterModel, value);
            }
        }
    }
}
