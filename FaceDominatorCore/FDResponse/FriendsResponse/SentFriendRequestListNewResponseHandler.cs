...
{
    public class SentFriendRequestListNewResponseHandler : FdResponseHandler, IResponseHandler
    {
        ...
        public SentFriendRequestListNewResponseHandler
            (IResponseParameter responseParameter, FriendsPager objPager, string userId, bool isExtraPager)
            : base(responseParameter)
        {
            ...
            try
            {
                ...
                HtmlNodeCollection objHtmlNodeCollection = objHtmlDocument.DocumentNode.SelectNodes("(//table[@class=\"uiGrid _51mz _5f0n\"])");
                ...
                if (!isFirstPage && (ObjFdScraperResponseParameters.ListUser.Count > 0
                                 || !string.IsNullOrEmpty(ObjFdScraperResponseParameters.FriendsPager.PrevousCursor)))
                    Status = true;
            ...
            try
            {
                ...
                if (!ObjFdScraperResponseParameters.FriendsPager.IsExtraPager)
                {
                    ...
                    if (string.IsNullOrEmpty(ObjFdScraperResponseParameters.FriendsPager.Data))
                    {
                        ...
                        if (int.TryParse(year, out yearInt) && int.TryParse(month, out monthInt))
                        {
                    ...
                    ObjFdScraperResponseParameters.FriendsPager.CurrentDataKey = currentKey;
                    ObjFdScraperResponseParameters.FriendsPager.Data = "{\"year\":" + year + ",\"month\":" + month + ",\"log_filter\":\"sentfriendrequests\",\"profile_id\":" + userId + "}";
                ...
                ObjFdScraperResponseParameters.FriendsPager.PrevousCursor = Uri.UnescapeDataString(FdFunctions.GetPrtialDecodedResponse(previousCursor));
                ...
                foreach (string friendItem in friendSuggetionList)
                {
                    try
                    {
                        ...
                        if (nodeCollection.Count > 1)
                        {
                            ...
                            {
                                friendId = Regex.Matches(nodeCollection[1].OuterHtml, "user.php\\?id=(.*?)&")[0].Groups[1].ToString();
                                ...
                                var friendNameDetails = Regex.Matches(partialDecodedResponse, $"id={friendId}(.*?)/a>")[0].Groups[1].ToString();

                                friendName = FdRegexUtility.FirstMatchExtractor(friendNameDetails, FdConstants.EntityNameRegex);

                                friendName = FdFunctions.GetPrtialDecodedResponse(friendName);
