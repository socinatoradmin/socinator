...
using AccountPageMembership = DominatorHouseCore.DatabaseHandler.FdTables.Accounts.PageMemberShip;
using CampaignInteractedPages = DominatorHouseCore.DatabaseHandler.FdTables.Campaigns.InteractedPages;
using CampaignPageMembership = DominatorHouseCore.DatabaseHandler.FdTables.Campaigns.PageMemberShip;

namespace FaceDominatorCore.FDLibrary.FdProcesses
{

    public class MessageToFanpageProcess : FdJobProcessInteracted<AccountInteractedPages>
    {
        public MessageToFanpagesModel MessageToFanpagesModel { get; set; }
        ...
        public MessageToFanpageProcess(IProcessScopeModel processScopeModel,
            ...
            : base(processScopeModel, accountServiceScoped, queryScraperFactory, fdHttpHelper, fdLogInProcess, dbCampaignServiceScoped)
        {
            ...
            MessageToFanpagesModel = processScopeModel.GetActivitySettingsAs<MessageToFanpagesModel>();
        ...
        public override JobProcessResult PostScrapeProcess(ScrapeResultNew scrapeResult)
        {
            ...
            try
            {
                ...
                KeyValuePair<string, string> message = GetMessageDetails(scrapeResult.QueryInfo, objFanpageDetails);
                ...
                var isSuccess = AccountModel.IsRunProcessThroughBrowser ?
                            FdLogInProcess._browserManager.SendMessage(AccountModel, objFanpageDetails.FanPageID, message.Key, MessageToFanpagesModel.IsMessageAsPreview,
                            scrapeResult.QueryInfo.QueryTypeEnum, message.Value, openWindow: true) : (MessageToFanpagesModel.IsMessageAsPreview ? FdRequestLibrary.SendTextMessageWithLinkPreview(AccountModel, objFanpageDetails.FanPageID, message.Key)
        ...
        private KeyValuePair<string, string> GetMessageDetails(QueryInfo queryInfo, FanpageDetails objFanpageDetails)
        {
            ...
            try
            {
                MessageToFanpagesModel.LstDisplayManageMessageModel.ForEach(x =>
                {
                    x.SelectedQuery.ForEach(y =>
                    {
                        if (y.Content.QueryTypeEnum == queryInfo.QueryTypeEnum && y.Content.QueryValue.Trim() == queryInfo.QueryValue)
                        {
                ...
                foreach (var currentMessage in lstMessage)
                {
                    ...
                    if (MessageToFanpagesModel.IsTagChecked)
                    {
                        textMessage = ReplaceTagWithValue(objFanpageDetails, textMessage);
                    }

                    if (MessageToFanpagesModel.IsSpintaxChecked)
                    {
        ...
        private void AddProfileScraperDataToDatabase(ScrapeResultNew scrapeResult, KeyValuePair<string, string> message)
        {
            try
            {
                ...
                bool isLikedByUser = page.IsLikedByUser;
                ...
                if (modulesetting.IsTemplateMadeByCampaignMode)
                {
                    ObjDbCampaignService.Add(new CampaignInteractedPages
                    {
                        ...
                        MembershipStatus = isLikedByUser
                            ? CampaignPageMembership.LikedPage : CampaignPageMembership.NotLiked
                ...
                ObjDbAccountService.Add(new AccountInteractedPages
                {
                    ...
                    InteractionDateTime = DateTime.Now,
                    MembershipStatus = isLikedByUser
                            ? AccountPageMembership.LikedPage : AccountPageMembership.NotLiked
                });
            }
            catch (Exception ex)
            {
