...
{
    public class TwitterAccountSessionManager: ITwitterAccountSessionManager
    {
        public List<AccountsSessionsTable> TwitterAccountSessions { get; set; }
        public string SessionFileName { get; set; } = "TwitterAccountSession.db";
        ...
        private readonly ITdHttpHelper httpHelper;
        public TwitterAccountSessionManager()
        {
            SessionFolder = ConstantVariable.GetIndexAccountDir() + $"\\DB\\{SessionFileName}";
            httpHelper = InstanceProvider.GetInstance<ITdHttpHelper>();
        ...
        public void AddOrUpdateSession(ref DominatorAccountModel dominatorAccount, bool update = false)
        {
            try
            {
                ...
                TwitterAccountSessions = Get<AccountsSessionsTable>();
                var Account = dominatorAccount;
                var ID = GetCurrentSession(Account);
                if (TwitterAccountSessions.Exists(x => x.AccountId == Account.AccountId && (x.CurrentSessionId == ID || x.CurrentSessionId == CurrentSessionId)))
                {
                    if (update)
                    {
                        var currentSession = TwitterAccountSessions.FirstOrDefault(x => x.AccountId == Account.AccountId && (x.CurrentSessionId == ID || x.CurrentSessionId == CurrentSessionId));
        ...
        public void InitializeAllSession()
        {
            try
            {
                ...
                var accounts = accountModels.LstDominatorAccountModel.Where(x => x.AccountBaseModel.AccountNetwork == DominatorHouseCore.Enums.SocialNetworks.Twitter).ToList();
                TwitterAccountSessions = Get<AccountsSessionsTable>();
                try
                {
                    //Remove 3 months Old Session.
                    var oldSessions = TwitterAccountSessions?.FindAll(x => (GetDayFromEpochTime(x.LastUpdated)) > 180);
                ...
                catch { TwitterAccountSessions = Get<AccountsSessionsTable>(); }
                if (accounts.Count >= TwitterAccountSessions?.Count && TwitterAccountSessions?.Count > 0)
                {
                    foreach (var item in accounts.SkipWhile(x => TwitterAccountSessions.Any(y => y.AccountId == x.AccountId)))
                    {
                        AddNewSession(item);
                    }
                }
