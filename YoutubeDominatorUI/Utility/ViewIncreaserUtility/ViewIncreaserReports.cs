...
{
    public class ViewIncreaserReports : IYdReportFactory
    {
        ...
        {
            return JsonConvert.DeserializeObject<ViewIncreaserModel>(activitySettings).SavedQueries;
        }
            ...
            List<KeyValuePair<string, string>> lstQueryDetails, CampaignDetails campaignDetails)
        {
            ...
            IDbCampaignService campaignDataBase = new DbCampaignService(campaignDetails.CampaignId);

            #region get data from InteractedUsers table and add to ViewIncreaserReportModel

            campaignDataBase.GetAllInteractedPosts().ForEach(
                report =>
                {
                    if (!string.IsNullOrWhiteSpace(report.ChannelId))
                    {
                        ...
                        {
                            Id = id++,
                            ActivityType = ActivityType.ViewIncreaser.ToString(),
                            ...
                            RepeatedWatchCount = report.ViewingCountPerAccount,
            ...
            reportModel.GridViewColumn =
                new ObservableCollection<GridViewColumnDescriptor>
                {
                    ...
                    new GridViewColumnDescriptor
                        {ColumnHeaderText = "Watch Count", ColumnBindingText = "RepeatedWatchCount"}
                };
            //});
        ...
        public IList GetsAccountReport(IDbAccountService dataBase)
        {
            ...
            IList reportDetails = dataBase.Get<InteractedPosts>()
                .Where(x => x.ActivityType == ActivityType.ViewIncreaser.ToString()).ToList();
            ...
            foreach (InteractedPosts item in reportDetails)
            {
                ...
                AccountsInteractedPosts.Add(
                    new InteractedPostsReport
                    {
                        ...
                        InteractionTime = time,
                        RepeatedWatchCount = item.ViewingCountPerAccount,
        ...
        public void ExportReports(ReportType reportType, string fileName)
        {
            ...
            if (reportType == ReportType.Account)
                ...
                {
                    csvData.Add(utilities.CreateCsvDataForPost(report, addRepeatedWatchCount: true));
                });
            ...
            Header = utilities.HeaderStringForPostReport(addRepeatedWatchCount: true);
