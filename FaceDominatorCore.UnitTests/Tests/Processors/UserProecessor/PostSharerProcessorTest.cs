...
{
    [TestClass]
    public class PostSharerProcessorTest : BaseFacebookProcessorTest
    {
        private PostSharerProcessor _sut;
        ...
        [TestMethod]
        public void PostSharersProcessor_Should_Execute_Successfully()
        {
      ...
      ("FaceDominatorCore.UnitTests.TestData.FdLibraryData.GetPostResponse.html", Assembly.GetExecutingAssembly());
            ...
            var responseHandler = new PostSharerResponseHandler(responseParameter, false)
            {
                Status = true
            };
            ...
            FdRequestLibrary.GetPostSharer(Arg.Any<DominatorAccountModel>(), Arg.Any<string>(), null)
                .Returns(responseHandler);

            _sut = new PostSharerProcessor
            ...
            FdRequestLibrary.Received(1)
                .GetPostSharer(Arg.Any<DominatorAccountModel>(), Arg.Any<string>(), null);
        ...
        [TestMethod]
        public void PostSharerProcessor_Should_Failed_Execute()
        {
            var queryInfo = new QueryInfo { QueryValue = "" };
            _fdJobProcess = new CommentLikerProcesss(ProcessScopeModel, DbAccountServiceScoped, FdQueryScraperFactory, FdHttpHelper, FdLoginProcess, FdRequestLibrary, DbCampaignServiceScoped);

            FdRequestLibrary.GetPostSharer(Arg.Any<DominatorAccountModel>(), Arg.Any<string>(), null)
                .Returns((PostSharerResponseHandler)null);
