...
{
    [TestClass]
    public class LocationUserProcessorTest : BaseFacebookProcessorTest
    {
        private LocationUserProcessor _sut;
        ...
        [TestMethod]
        public void LocationUserProcessor_Should_Execute_Successfully()
        {
      ...
      ("FaceDominatorCore.UnitTests.TestData.FdLibraryData.searchPeopleGraphResponse.html", Assembly.GetExecutingAssembly());
            ...
            FdRequestLibrary.SearchPeopleByLocation(Arg.Any<DominatorAccountModel>(),
                Arg.Any<string>(), null).Returns(responseHandler);

            _sut = new LocationUserProcessor
            ...
            // assert
            FdRequestLibrary.Received(1).SearchPeopleByLocation(Arg.Any<DominatorAccountModel>(),
                Arg.Any<string>(), null);
        ...
        [TestMethod]
        public void LocationUserProcessor_Should_Failed_To_Execute()
        {
            ...
            FdRequestLibrary.SearchPeopleByLocation(Arg.Any<DominatorAccountModel>(), Arg.Any<string>(), null)
                .Returns((FdSearchPeopleResponseHandler)null);
            // act
            _sut.Start(queryInfo);

            // assert
            FdRequestLibrary.Received(1).SearchPeopleByLocation(Arg.Any<DominatorAccountModel>(), Arg.Any<string>(), null);
        }
    }
}
