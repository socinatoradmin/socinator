...
{
    [TestClass]
    public class SendFriendRequestProcessTest : BaseFacebookProcessTest
    {
        private SendFriendRequestProcess _sut;
        ...
        public override void SetUp()
        {
            ...
            SendFriendRequestModel sendFriendRequestModel = new SendFriendRequestModel();
            ProcessScopeModel.GetActivitySettingsAs<SendFriendRequestModel>().Returns(sendFriendRequestModel);

            _sut = new SendFriendRequestProcess(ProcessScopeModel, DbAccountServiceScoped, FdQueryScraperFactory, FdHttpHelper, FdLoginProcess, FdRequestLibrary, DbCampaignServiceScoped);

        }

        [TestMethod]
        public void Should_SendFriendRequest_And_Save_Result_In_Database()
        {
            ...
            FdRequestLibrary.SendFriendRequest
                (Arg.Any<DominatorAccountModel>(), Arg.Any<string>()).Returns("success");

            // act
            var jpr = _sut.PostScrapeProcess(_scrapeResultNew);

            // assert
            FdRequestLibrary.Received(1).SendFriendRequest
                (Arg.Any<DominatorAccountModel>(), Arg.Any<string>());
