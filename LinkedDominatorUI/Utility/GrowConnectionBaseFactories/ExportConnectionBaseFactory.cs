...
{
    public class ExportConnectionBaseFactory : ILdBaseFactory
    {
        public ILdUtilityFactory LdUtilityFactory()
        {
            ...
            var builder = new LdBaseFactoryBuilder(utilityFactory)
                .AddModuleName(ActivityType.ExportConnection)
                .AddReportFactory(new ExportConnectionReport())
                .AddViewCampaignFactory(new ExportConnectionViewCampaign());

            return builder.LdUtilityFactory;
        }
    }

    public class ExportConnectionReport : ILdReportFactory
    {
        ...
        public string activityType = ActivityType.ExportConnection.ToString();
        public string Header { get; set; } = string.Empty;

        public ObservableCollection<QueryInfo> GetSavedQuery(ActivityType subModuleName, string activitySettings)
        {
            return JsonConvert.DeserializeObject<ExportConnectionModel>(activitySettings).SavedQueries;
        }
        ...
        public void ExportReports(ActivityType activityType, string FileName, ReportType reportType)
        {
            ...
            {
                Header =
                    "AccountEmail,ActivityType,UserFullName,UserProfileUrl,PhoneNumber,EmailAddress,InteractionDateTime";

                InteractedUsersReportModel.ToList().ForEach(ReportItem =>
                {
                    try
                    {
                        #region PhoneNumber and EmailAddress

                        var PhoneNumber = string.Empty;
                        var EmailAddress = string.Empty;
                        try
                        {
                            PhoneNumber = Regex.Split(ReportItem.DetailedUserInfo, "<:>")[0];
                        }
                        catch (Exception ex)
                        {
                        }
                        ...
                        try
                        {
                            EmailAddress = Regex.Split(ReportItem.DetailedUserInfo, "<:>")[1];
                        }
                        catch (Exception ex)
                        {
                        }

                        if (string.IsNullOrEmpty(PhoneNumber)) PhoneNumber = "N/A";
                        if (string.IsNullOrEmpty(EmailAddress)) EmailAddress = "N/A";

                        #endregion

                        CsvData.Add(ReportItem.AccountEmail + "," + ReportItem.ActivityType + "," +
                                    ReportItem.UserFullName + "," +
                                    ReportItem.UserProfileUrl + "," + PhoneNumber + "," + EmailAddress + "," +
            ...
            {
                Header = "ActivityType,UserFullName,UserProfileUrl,PhoneNumber,EmailAddress,InteractionDateTime";
    ...
    public class ExportConnectionViewCampaign : ILdViewCampaign
    {
            ...
            bool IsEditCampaignName, Visibility CancelEditVisibility, string CampaignButtonContent, string TemplateID)
        {
            try
            {
                var objExportConnection = ExportConnection.GetSingeltonObjectExportConnection();
                objExportConnection.IsEditCampaignName = IsEditCampaignName;
                objExportConnection.CancelEditVisibility = CancelEditVisibility;
                objExportConnection.TemplateId = TemplateID;
                //objExportConnection.CampaignName = campaignDetails.CampaignName;
                objExportConnection.CampaignName = CampaignButtonContent == ConstantVariable.UpdateCampaign()
                    ? campaignDetails.CampaignName
                    : objExportConnection.CampaignName;
                objExportConnection.CampaignButtonContent = CampaignButtonContent;
                objExportConnection.SelectedAccountCount = campaignDetails.SelectedAccountList.Count +
                                                           $" {"LangKeyAccountSelected".FromResourceDictionary()}";
                objExportConnection.ExportConnectionFooter.list_SelectedAccounts = campaignDetails.SelectedAccountList;

                objExportConnection.ObjViewModel.ExportConnectionModel =
                    templateDetails.ActivitySettings.GetActivityModel<ExportConnectionModel>(
                        objExportConnection.ObjViewModel.Model, true);

                objExportConnection.MainGrid.DataContext = objExportConnection.ObjViewModel;
