...
{
    public class CommentBaseFactory : ILdBaseFactory
    {
    ...
    public class CommentReport : ILdReportFactory
    {
        ...
        public string ActivityType = DominatorHouseCore.Enums.ActivityType.Comment.ToString();
            ...
            List<KeyValuePair<string, string>> lstQueryDetails, CampaignDetails campaignDetails)
        {
            ...
            dataBase.GetInteractedPosts(ActivityType).ForEach(
                reportItem =>
                {
                    InteractedPostsReportModel.Add(new InteractedPostsReportModel
                    {
                        ...
                        MyComment = reportItem.MyComment,
            ...
            reportModel.GridViewColumn =
                new ObservableCollection<GridViewColumnDescriptor>
                {
                    ...
                    // new GridViewColumnDescriptor {ColumnHeaderText = "Post Title",ColumnBindingText="PostTitle" },
                    new GridViewColumnDescriptor
                    {
                    ...
                    new GridViewColumnDescriptor
                    {
                        ColumnHeaderText = "LangKeyMyComment".FromResourceDictionary(), ColumnBindingText = "MyComment"
                    },
                    new GridViewColumnDescriptor
                    {
        ...
        public void ExportReports(ActivityType activityType, string fileName, ReportType reportType)
        {
            ...
            switch (reportType)
            {
                case ReportType.Campaign:
                    Header = utils.PostsReportHeader(addMyComment: true);
                    //"Account Email,Query Type,Query Value,Activity Type,Post Owner FullName,Post Owner Url,Post Link,Post Description,My Comment,Like Count,Comment Count,Interaction DateTime";

                    InteractedPostsReportModel.ToList().ForEach(reportItem =>
                    {
                        try
                        {
                            csvData.Add(utils.PostsReportCSVData(reportItem, addMyComment: true));
                            //csvData.Add(reportItem.AccountEmail + "," + reportItem.QueryType + "," + reportItem.QueryValue + "," +
                            //            reportItem.ActivityType + "," + reportItem.PostOwnerFullName + "," + reportItem.PostOwnerProfileUrl + "," + reportItem.PostLink + "," +
                            //             reportItem.PostDescription + "," + reportItem.MyComment + "," + reportItem.LikeCount + "," + reportItem.CommentCount + "," +
                ...
                case ReportType.Account:
                    Header = utils.PostsReportHeader(false, true);
                    //"Query Type,Query Value,Activity Type,Post Owner FullName,Post Owner Url,Post Link,Post Description,My Comment,Like Count,Comment Count,Interaction DateTime";

                    AccountsInteractedPosts.ToList().ForEach(reportItem =>
                    {
                        try
                        {
                            csvData.Add(EnagageReportUtils.ReportCSVData(reportItem, true));
                            ...
                            //              reportItem.PostLink + "," + reportItem.PostDescription + "," + reportItem.MyComment + "," + reportItem.LikeCount + "," + reportItem.CommentCount + "," +
                            //            reportItem.InteractionDatetime);
                        }
                        catch (Exception ex)
                        {
    ...
    public class CommentViewCampaign : ILdViewCampaign
    {
