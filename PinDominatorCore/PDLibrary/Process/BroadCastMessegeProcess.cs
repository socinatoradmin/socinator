...
{
    public class BroadCastMessegeProcess : PdJobProcessInteracted<InteractedUsers>
    {
        ...
        public BroadCastMessegeProcess(IProcessScopeModel processScopeModel,
                ...
                pdhttpHelper, pdLogInProcess)
        {
            ...
            _broadcastMessagesModel = JsonConvert.DeserializeObject<BroadcastMessagesModel>(templatesFileManager.Get()
        ...
        public override JobProcessResult PostScrapeProcess(ScrapeResultNew scrapeResult)
        {
            ...
            try
            {
                ...
                foreach (var item in _broadcastMessagesModel.LstDisplayManageMessagesModel)
                ...
                if (_broadcastMessagesModel.IsMakeMessageAsSpinText)
                ...
                if (DominatorAccountModel.IsRunProcessThroughBrowser)
                    ...
                    DominatorAccountModel, _broadcastMessagesModel.IsSendPinAsAMessage);
                ...
                else
                {
                    ...
                    if (_broadcastMessagesModel.IsChkStopActivityAfterXXFailed && _activityFailedCount++ == _broadcastMessagesModel.ActivityFailedCount)
                    {
                        GlobusLogHelper.log.Info(Log.CustomMessage, DominatorAccountModel.AccountBaseModel.AccountNetwork,
                            DominatorAccountModel.AccountBaseModel.UserName, ActivityType,
                            $"{"LangKeyStopActivityAfter".FromResourceDictionary()} {_activityFailedCount - 1} " +
                            $"{"LangKeyFailedAttemptFor".FromResourceDictionary()} {_broadcastMessagesModel.FailedActivityReschedule} " +
                            $"{"LangKeyHour".FromResourceDictionary()}");

                        StopAndRescheduleJob(_broadcastMessagesModel.FailedActivityReschedule);
                    }
                    var message = response?.Issue?.Message;
                    if(string.IsNullOrEmpty(message) || message.Contains("Unable to send message because of recipient's message settings"))
                        message = "Message option is not available";
