...
{
    public class UnLikeReports : ITumblrReportFactory
    {
        public static ObservableCollection<UnLikeReportDetails> InteractedUsersModel =
            new ObservableCollection<UnLikeReportDetails>();

        public static ObservableCollection<UnLikeReportDetails> AccountsInteractedUsers =
            new ObservableCollection<UnLikeReportDetails>();
            ...
            List<KeyValuePair<string, string>> lstQueryDetails, CampaignDetails campaignDetails)
        {
            ...
            dataBase.GetAllUnLikedPosts().ForEach(
                report =>
                {
                    InteractedUsersModel.Add(new UnLikeReportDetails
                    {
                        Id = report.Id,
            ...
            reportModel.GridViewColumn =
                new ObservableCollection<GridViewColumnDescriptor>
                {
                    ...
                    new GridViewColumnDescriptor {ColumnHeaderText = "UnLiked Url", ColumnBindingText = "PostUrl"},
        ...
        public void ExportReports(ReportType dataSelectionType, string fileName)
        {
            ...
            if (reports == ReportType.Campaign)
            {
                ...
                InteractedUsersModel.ToList().ForEach(report =>
                {
                    try
                    {
                        csvData.Add(report.AccountName + "," + report.PostUrl + "," + report.Date);
                    }
                    catch (Exception ex)
                    {
        ...
        {
            var forLocalTime = DateTime.Now - DateTime.UtcNow;
            IList reportDetails = dataBase.GetUnLikedUsers(ActivityType.Unlike).ToList();

            foreach (UnLikedPosts report in reportDetails)
            {
                _i = _i + 1;
                AccountsInteractedUsers.Add(
                    new UnLikeReportDetails
                    {
                        Id = _i,
                        AccountName = report.UserName,
                        Date = report.InteractionTimeStamp.EpochToDateTimeUtc() + forLocalTime,
