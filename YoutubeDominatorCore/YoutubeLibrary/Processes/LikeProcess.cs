...
{
    public class LikeProcess : YdJobProcessInteracted<InteractedPosts>
    {
        ...
        public LikeProcess(IProcessScopeModel processScopeModel,
            ...
            : base(processScopeModel, accountServiceScoped, globalService, executionLimitsManager,
                queryScraperFactory, ydHttpHelper, ydLogInProcess)
        {
            _campaignService = campaignService;
            _accountServiceScoped = accountServiceScoped;
            _youtubeFunctionality = youtubeFunctionality;
            EngageLikeModel = processScopeModel.GetActivitySettingsAs<LikeModel>();
        }

        public LikeModel EngageLikeModel { get; set; }
        ...
        public override JobProcessResult PostScrapeProcess(ScrapeResultNew scrapeResult)
        {
            ...
            try
            {
                ...
                {
                    FailedLog(youtubePost.Reaction == InteractedPosts.LikeStatus.Like
                        ? $"- Already liked {youtubePost.PostUrl}"
                        : youtubePost.PostUrl);
                    jobProcessResult.IsProcessSuceessfull = false;
                }
                if (EngageLikeModel != null && EngageLikeModel.EnableDelayBetweenPerformingActionOnSamePost)
                    DelayBeforeNextActivity(EngageLikeModel.DelayBetweenPerformingActionOnSamePost.GetRandom());
                else
        ...
        public void AddToDataBase(ScrapeResultNew scrapeResult)
        {
            try
            {
                var youtubePost = (YoutubePost)scrapeResult.ResultPost;
                ...
                if (!string.IsNullOrEmpty(CampaignId))
                {
                    ...
                    existing.ActivityType = ActivityType.Like.ToString();
                    ...
                    existing.ReactionOnPost = InteractedPosts.LikeStatus.Like;
                    existing.PostDescription = youtubePost.Caption ?? "";
                    existing.PublishedDate = youtubePost.PublishedDate ?? "";
                    existing.QueryType = scrapeResult.QueryInfo.QueryType ?? "";
                    existing.QueryValue = scrapeResult.QueryInfo.QueryValue ?? "";
                    ...
                    if (string.IsNullOrWhiteSpace(existing.PostTitle))
                    {
                    }
