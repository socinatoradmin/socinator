...
namespace DominatorHouseCore.Utility
{
    public class MediaUtilites
    {
        ...
        public static bool IsRequiredMimeType(string mimeType)
        {
            if (!string.IsNullOrEmpty(mimeType)
                &&
                (mimeType == MimeType.Html.GetDescriptionAttr()
                || mimeType == MimeType.JavaSript.GetDescriptionAttr()
                || mimeType == MimeType.JavaSriptText.GetDescriptionAttr()
                || mimeType == MimeType.BZipArchive.GetDescriptionAttr()
                || mimeType == MimeType.Json.GetDescriptionAttr()
                || mimeType == MimeType.JsonLd.GetDescriptionAttr()
                || mimeType == MimeType.XHTML.GetDescriptionAttr()
                || mimeType == MimeType.JavaScriptApp.GetDescriptionAttr()))
                return true;
            return false;
        }
        ...
        {
            var media = Path.Combine(Path.GetDirectoryName(input), Path.GetFileNameWithoutExtension(input));
        ...
        public static string GetFileExtension(string mediaFile)
        {
            try
            {
                return new FileInfo(mediaFile).Extension;
            }
            catch (Exception)
            {
                return string.Empty;
