...
{
    [TestClass]
    public class OwnFollowerProcessorTest:BaseGdProcessTest
    {
        ...
        [TestMethod]
        public void should_search_For_OwnFollower()
        {
            // arrange    
         ...
         "GramDominatorCore.UnitTests.TestData.OwnFollowerList.json", Assembly.GetExecutingAssembly());
            ...
            //act      
            OwnFollowersProcessor OwnFollower = new OwnFollowersProcessor(_gdJobProcess, AccountServiceScoped,
                CampaignService, ProcessScopeModel, DelayService);
            OwnFollower.Start(queryInfo);
            //assert
            _gdJobProcess.Received(174).FinalProcess(Arg.Any<ScrapeResultNew>());
        }
    }
}
