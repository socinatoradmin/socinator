...
{
    [TestClass]
    public class UserWhoCommentedOnPostProcessorTest: BaseGdProcessTest
    {
        ...
        [TestMethod]
        [Timeout(15000)]
        public void should_search_For_UserWhoCommentedOnPost()
        {
            // arrange    
            var queryInfo = new QueryInfo
            {
                QueryType = "Users Who Commented On Posts",
            ...
            var CommentedResponse = TestUtils.ReadFileFromResources(
               "GramDominatorCore.UnitTests.TestData.GetMediaCommentsResponse.json", Assembly.GetExecutingAssembly());
            MediaCommentsIgResponseHandler GetMediaCommentResponse = new MediaCommentsIgResponseHandler(new ResponseParameter()
            {
                Response = CommentedResponse
            });
            //act
            _gdJobProcess.instaFunct.GetMediaComments(Arg.Any<DominatorAccountModel>(), Arg.Any<string>(), Arg.Any<CancellationToken>(), Arg.Any<string>()).Returns(GetMediaCommentResponse);
            UsersWhoCommentedOnPostProcessor UserWhoCommentedOnPost =
                new UsersWhoCommentedOnPostProcessor(_gdJobProcess, AccountServiceScoped, CampaignService,
                    ProcessScopeModel, DelayService);
            UserWhoCommentedOnPost.Start(queryInfo);
            //assert
            _gdJobProcess.Received(20).FinalProcess(Arg.Any<ScrapeResultNew>());
            _gdJobProcess.Received(1).instaFunct.GetMediaComments(Arg.Any<DominatorAccountModel>(), Arg.Any<string>(), Arg.Any<CancellationToken>());
