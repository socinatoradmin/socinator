...
{
    internal class CommentReport : ITDReportFactory
    {
                ...
                dboperation.GetAllInteractedPosts().ForEach(
                    report =>
                    {
                        ScrapeTweetReportModel.Add(new InteractedTweetReport
                        {
                            ...
                            TweetedText = report.CommentedText,
            ...
            ObjReports.GridViewColumn =
                new ObservableCollection<GridViewColumnDescriptor>
                {
                    ...
                    new GridViewColumnDescriptor
                        {ColumnHeaderText = "Commented Tweet Id", ColumnBindingText = "TweetId"},
                    new GridViewColumnDescriptor
                        {ColumnHeaderText = "Commented Text", ColumnBindingText = "TweetedText"},
                    ...
                    new GridViewColumnDescriptor
                        {ColumnHeaderText = "Liked Date", ColumnBindingText = "InteractionDate"}
                };
            //  });
        ...
        public IList GetsAccountReport(IDbAccountService dataBase)
        {
            ...
            dataBase.GetInteractedPosts(ActivityType.Comment).Where(x => x.QueryType != null)?.ForEach(
                report =>
                {
            ...
            //  CsvHeader = "Sl No,Query Type,Query Value,Tweet Id,Commented Text,Tweeted Text,Tweet Owner,Tweeted Date, Media Path,Retweet,Follow Status,Follow Back Status,Process Type,Reposted Date";
            //  break;
        ...
        public void ExportReports(string fileName, ReportType reportType)
        {
            ...
            {
                Header =
                    "Sl No,Account,Query Type,Query Value,Commented on Tweet Id,Commented Text,Tweet Owner,Retweet,Following,Following Back,Commented Date";
                ...
                ScrapeTweetReportModel.ToList().ForEach(report =>
                {
                    try
                    {
                        csvData.Add(report.SlNo + "," + report.SinAccUsername + "," + report.QueryType + "," +
                                    ...
                                    report.CommentedText + report.InteractionDate);
                        ;
                    }
                    catch (Exception ex)
                    {
            ...
            {
                Header =
                    "Sl No,Query Type,Query Value,Tweet Id,Commented Text,Tweeted Text,Tweet Owner,Tweeted Date, Media Path,Retweet,Follow Status,Follow Back Status,Process Type,Reposted Date";
                ...
                AccountsInteractedUsers.ToList().ForEach(x =>
                {
                    try
                    {
                        csvData.Add(x.SlNo + "," + x.QueryType + "," + x.QueryValue + ",'" + x.TweetId + "',\"" +
                                    x.CommentedText.Replace("\"", "\"\"") + "\",\"" +
                                    x.TweetedText.Replace("\"", "\"\"") + "\","
                                    + x.TweetOwnerName + "," + x.TweetedDate + ",\"" +
                                    x.MediaPath.Replace("\"", "\"\"") + "\"," + x.Retweet + "," + x.FollowStatus + "," +
