...
{
    internal class DeleteReport : ITDReportFactory
    {
        ...
        {
            return JsonConvert.DeserializeObject<DeleteModel>(activitySettings).SavedQueries;
        }
        ...
        public ObservableCollection<object> GetCampaignsReport(ReportModel ObjReports, CampaignDetails campaignDetails)
        {
            ...
            // ObjReports.ReportCollection = CollectionViewSource.GetDefaultView(ScrapeTweetReportModel);
            ...
            ObjReports.GridViewColumn =
                new ObservableCollection<GridViewColumnDescriptor>
                {
                    ...
                    new GridViewColumnDescriptor {ColumnHeaderText = "Delete Type", ColumnBindingText = "QueryType"},
                    ...
                    new GridViewColumnDescriptor
                        {ColumnHeaderText = "Deleted Date", ColumnBindingText = "InteractionDate"}
                };
        ...
        public IList GetsAccountReport(IDbAccountService dataBase)
        {
            ...
            dataBase.GetInteractedPosts(ActivityType.Delete)?.ForEach(
                report =>
                {
        ...
        public void ExportReports(string fileName, ReportType reportType)
        {
            ...
            {
                Header = "Sl No,Account,Delete Type,Tweet Id,Tweeted Text,Tweeted Date,Deleted Date";
                ...
                ScrapeTweetReportModel.ToList().ForEach(report =>
                {
                    try
                    {
                        csvData.Add(report.SlNo + "," + report.SinAccUsername + "," + report.QueryType
                                    + ",'" + report.TweetId + "'," + report.TweetedText + "," + report.TweetedDate
            ...
            {
                Header = "Sl No,Query Type,Tweet Id,Tweeted Text,Tweeted Date, Deleted Date";

                AccountsInteractedUsers.ToList().ForEach(x =>
                {
                    try
                    {
                        csvData.Add(x.SlNo + "," + x.QueryType + ",'" + x.TweetId + "'," + x.TweetedText + "," +
                                    x.TweetedDate + "," + x.InteractionDate.ToString());
                    }
                    catch (Exception ex)
                    {
