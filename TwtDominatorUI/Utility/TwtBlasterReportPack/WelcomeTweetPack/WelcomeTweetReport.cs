...
{
    internal class WelcomeTweetReport : ITDReportFactory
    {
        ...
        {
            return JsonConvert.DeserializeObject<WelcomeTweetModel>(activitySettings).SavedQueries;
        }
            ...
            ObjReports.GridViewColumn =
                new ObservableCollection<GridViewColumnDescriptor>
                {
                    ...
                    new GridViewColumnDescriptor
                        {ColumnHeaderText = "Tweeted Date", ColumnBindingText = "InteractionDate"}
                    //  new GridViewColumnDescriptor { ColumnHeaderText = "Tweet Owner", ColumnBindingText ="TweetOwnerName"},
                    ...
                    //  new GridViewColumnDescriptor { ColumnHeaderText = "Retweeted Date", ColumnBindingText ="InteractionDate"},
                };
            //   });
        ...
        public IList GetsAccountReport(IDbAccountService dataBase)
        {
            ...
            dataBase.GetInteractedPosts(ActivityType.WelcomeTweet)?.ForEach(
                report =>
                {
        ...
        public void ExportReports(string fileName, ReportType reportType)
        {
            ...
            {
                Header = "Sl No,Account,Query Type,Tweet Id,Tweeted Text,Tweeted Date";
            ...
            {
                Header = "Sl No,Query Type,Tweet Id,Tweeted Text,Tweeted Date";

                AccountsInteractedUsers.ToList().ForEach(report =>
                {
                    try
                    {
                        csvData.Add(report.SlNo + "," + report.QueryType + ",'" + report.TweetId + "',\"" +
                                    report.TweetedText.Replace("\"", "\"\"") + "\"," + report.InteractionDate);
                        ;
                    }
                    catch (Exception ex)
                    {
