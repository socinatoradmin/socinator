...
{
    public class AutoReplyToNewMessageReports : IPdReportFactory
    {
        ...
        //public List<UserReportAccountModel> AccountsInteractedUsers = new List<UserReportAccountModel>();
            ...
            List<KeyValuePair<string, string>> lstQueryDetails, CampaignDetails campaignDetails)
        {
            ...
            dbCampaignService.Get<InteractedUsers>(x => x.ActivityType == autoReplyToNewMessageActivity).ForEach(
                report =>
                {
        ...
        public void ExportReports(ReportType dataSelectionType, string fileName)
        {
            ...
            {
                Header =
                    "Query,Query Type,Followed Back,Activity Type,Username,User Id,Interaction Time,Full Name,Followers Count,Followings Count,Pins Count,Tries Count,Website,Bio,Has Anonymous Profile Picture,Profile Pic Url";
                ...
                LstInteractedUserAccount.ToList().ForEach(report =>
                {
                    try
                    {
                        csvData.Add(report.Query + "," + report.QueryType + "," + report.FollowedBack + "," +
                                    ...
                                    "," + report.TriesCount + ","
                                    + report.Website + "," + report.Bio?.Replace(',', '.') + "," +
            ...
            {
                Header =
                    "Id,Query,Query Type,Followed Back,Interaction Time,Activity Type,Username,Interacted Username,Interacted User Id,Followers Count,Followings Count,Pins Count,Full Name,Has Anonymous Profile Picture,Profile Pic Url,Bio,Website,Message";
                ...
                InteractedUsersReportModel.ToList().ForEach(report =>
                {
                    try
                    {
                        csvData.Add(report.Id + "," + report.Query + "," + report.QueryType + "," +
                                    report.FollowedBack + ","
                                    + report.InteractionTime + "," + report.ActivityType + "," + report.Username + "," +
                                    report.InteractedUsername + ","
                                    + report.InteractedUserId + "," + report.FollowersCount + "," +
                                    report.FollowingsCount + "," + report.PinsCount + ","
                                    + report.FullName + "," + report.HasAnonymousProfilePicture + "," +
                                    report.ProfilePicUrl + ","
                                    + report.Bio?.Replace(',', '.') + "," + report.Website+","+report.DirectMessage?.Replace(",","."));
                    }
                    catch (Exception ex)
                    {
                        ex.DebugLog();
