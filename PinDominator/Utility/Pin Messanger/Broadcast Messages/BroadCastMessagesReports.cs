...
{
    public class BroadCastMessagesReports : IPdReportFactory
    {
        ...
        private static readonly List<InteractedUsersReportDetails> LstInteractedUserAcccount =
            ...
            List<KeyValuePair<string, string>> lstQueryDetails, CampaignDetails campaignDetails)
        {
            ...
            dbCampaignService.Get<InteractedUsers>(x => x.ActivityType == broadcastMessagesActivity).ForEach(
                report =>
                {
        ...
        {
            LstInteractedUserAcccount.Clear();
            ...
            return LstInteractedUserAcccount.Select(x =>
                new
                {
                    x.Id,
                    x.Query,
        ...
        public void ExportReports(ReportType dataSelectionType, string fileName)
        {
            ...
            {
                Header =
                    "Query,QueryType,Followed Back,Activity Type,Interacted Username,Interacted User Id,Interaction Time,Full Name," +
                    "Followers Count,Followings Count,Pins Count,Has Anonymous Profile Picture,Profile Pic Url";

                LstInteractedUserAcccount.ToList().ForEach(report =>
                {
                    try
                    {
            ...
            {
                Header =
                    "Username,Query,QueryType,Followed Back,Activity Type,Interacted Username,Interacted User Id,Interaction Time,Full Name," +
                    "Followers Count,Followings Count,Pins Count,Has Anonymous Profile Picture,Profile Pic Url";
                ...
                InteractedUsersReportModel.ToList().ForEach(report =>
                {
                    try
                    {
                        report.Username = string.IsNullOrEmpty(report.Username) ? report.SinAccUsername : report.Username;
