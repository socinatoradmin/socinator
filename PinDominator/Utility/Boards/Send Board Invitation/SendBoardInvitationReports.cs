...
{
    public class SendBoardInvitationReports : IPdReportFactory
    {
        ...
        {
            return JsonConvert.DeserializeObject<SendBoardInvitationModel>(activitySettings).SavedQueries;
        }
            ...
            List<KeyValuePair<string, string>> queryDetails, CampaignDetails campaignDetails)
        {
            ...
            var id = 1;
            dbCampaignService.Get<InteractedBoards>(x => x.OperationType == ActivityType.SendBoardInvitation).ForEach(
                report =>
                {
        ...
        public IList GetsAccountReport(IDbAccountService dataBase)
        {
            IList reportDetails = dataBase.Get<DominatorHouseCore.DatabaseHandler.PdTables.Accounts.InteractedBoards>()
                .Where(x => x.OperationType == ActivityType.SendBoardInvitation).ToList();
