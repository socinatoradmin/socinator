...
{
    public class AcceptBoardInvitationReports : IPdReportFactory
    {
        ...
        {
            return JsonConvert.DeserializeObject<AcceptBoardInvitationModel>(activitySettings).SavedQueries;
        }
            ...
            List<KeyValuePair<string, string>> queryDetails, CampaignDetails campaignDetails)
        {
            ...
            var id = 1;
            dbCampaignService.Get<InteractedBoards>(x => x.OperationType == ActivityType.AcceptBoardInvitation).ForEach(
                report =>
                {
        ...
        public IList GetsAccountReport(IDbAccountService dataBase)
        {
            IList reportDetails = dataBase.Get<DominatorHouseCore.DatabaseHandler.PdTables.Accounts.InteractedBoards>()
                .Where(x => x.OperationType == ActivityType.AcceptBoardInvitation).ToList();
        ...
        public void ExportReports(ReportType dataSelectionType, string fileName)
        {
            ...
            {
                Header =
                    "Username,BoardId,BoardName,BoardDescription,InteractionDate,PinCount,FollowerCount,OperationType,UserId";

                LstInteractedBoardAccount.ToList().ForEach(report =>
                {
                    try
                    {
                        csvData.Add(report.Username + "," + report.BoardId + "," + report.BoardName + "," +
            ...
            {
                Header =
                    "Username,Interacted Username,BoardId,BoardName,BoardDescription,InteractionDate,PinCount,FollowerCount,OperationType,UserId";

                InteractedBoardsReportModel.ToList().ForEach(report =>
                {
                    try
                    {
                        csvData.Add(report.Username + "," + report.Username + "," + report.BoardId + "," +
                                    report.BoardName + "," + report.BoardDescription?.Replace(',', '.') + "," +
