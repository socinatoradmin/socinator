...
{
    public class EditPinReports : IPdReportFactory
    {
        ...
        {
            return JsonConvert.DeserializeObject<EditPinModel>(activitySettings).SavedQueries;
        }
            ...
            List<KeyValuePair<string, string>> queryDetails, CampaignDetails campaignDetails)
        {
            ...
            var id = 1;
            var actEditPin = ActivityType.EditPin.ToString();
            var editPinActInt = ((int) ActivityType.EditPin).ToString();
            dbCampaignService.Get<InteractedPosts>(x =>
                x.OperationType == actEditPin || x.OperationType == editPinActInt).ForEach(
                report =>
                {
        ...
        {
            var actType = ActivityType.EditPin.ToString();
            var editPinActInt = ((int) ActivityType.EditPin).ToString();
            IList reportDetails = dataBase.Get<DominatorHouseCore.DatabaseHandler.PdTables.Accounts.InteractedPosts>()
                .Where(x => x.OperationType == actType || x.OperationType == editPinActInt).ToList();
