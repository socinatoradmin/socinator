...
{
    public class MessageToFanpagesReport : IFdReportFactory
    {
        ...
        private readonly string _activityType = ActivityType.MessageToFanpages.ToString();
        public string Header { get; set; } = string.Empty;

        public ObservableCollection<QueryInfo> GetSavedQuery(ActivityType subModuleName, string activitySettings)
        {
            return JsonConvert.DeserializeObject<MessageToFanpagesModel>(activitySettings).SavedQueries;
        }
            ...
            dataBase.GetAllInteractedData<InteractedPages>().ForEach(
                report =>
                {
                    InteractedPageModel.Add(new PageReportModel
                    {
                        ...
                        UploadedMediaPath = report.UploadedMediaPath,
                ...
                try
                {
                    AccountsInteractedPages.Add(
                        new PageReportAccountModel
                        {
                            ...
                            PageName = report.PageName,
                            PageUrl = report.PageUrl,
                            Message = report.MessageText?.Replace(",", string.Empty)?.Replace("\r\n", " ")
                                ?.Replace("\n", " "),
                            UploadedMediaPath = report.UploadedMediaPath,
                            PageType = report.PageType,
                            TotalLikers = report.TotalLikers,
                    ...
                    if (!Data.Contains(objFanpageDetails.FanPageID))
                        Data.Add(report.QueryType + ","
                                                  ...
                                                  + report.MessageText?.Replace(",", string.Empty)?.Replace("\r\n", " ")
                                                      ?.Replace("\n", " ") + ","
        ...
        public void ExportReports(ActivityType activityType, string fileName, ReportType reportType)
        {
            ...
            {
                //Header = "AccountEmail,QueryType,QueryValue,Page Name,Page Url,Message, Media Path,Total Likers,Rating Count,Rating Value,Is Liked, Is Verified,Phone Number, Page Category,Page Description,Follower Count,Main Category, Address,Date";
            ...
            {
                //Header = "QueryType,QueryValue,Page Name,Page Url,Message, Media Path,Total Likers,Rating Count,Rating Value,Is Liked, Is Verified,Phone Number, Page Category,Page Description,Follower Count,Main Category, Address,Date";
